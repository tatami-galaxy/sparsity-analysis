...
from src.eval.run_math_eval import extract_boxed_answer, is_equiv, SYSTEM_PROMPT as EVAL_SYSTEM_PROMPT
...
def prepare_openthoughts(args, tokenizer) -> "Dataset":
    """Load OpenThoughts-114k (math), format for SFT, and filter by sequence length."""
    ds = load_openthoughts(max_samples=args.max_samples, seed=args.seed)
    print(f"Loaded {len(ds)} training examples from openthoughts (math domain)")
...
DATASET_PREPARERS = {
    ...
    "openthoughts": prepare_openthoughts,
}
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["numinamath", "competition_math", "deepmath", "openthoughts"],
